...
loss_types = ['B', 'A', 'C', 'M', 'P', 'D', 'E', 'S', 'I', 'N', 'M1', 'M2', 'M3', 'M4', 'M5', 'B1', 'B2', 'B3', 'B4', 'B5', 'E1', 'E2', 'E3', 'E4', 'E5']
...
class NetLoss(nn.Module):
    ...
    def __init__(self, net:model, criterion:CEL):
        super().__init__()
...
def train():
    ...
    dataset = ReferDataset(image_path=cfg.train_images,
    ...
    if args.resume is not None:
        ...
        cef_net.load_weights(args.resume)
    ...
    if not cfg.freeze_bn: cef_net.freeze_bn() # Freeze bn so we don't kill our means
    cef_net(torch.zeros(2, 3, cfg.max_size, cfg.max_size).cuda(), torch.ones(2, 20).cuda().long())
    if not cfg.freeze_bn: cef_net.freeze_bn(True)
    ...
    data_loader = data.DataLoader(dataset, args.batch_size,
                                  ...
                                  pin_memory=True, generator=torch.Generator('cuda'))
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if iteration % args.save_interval == 0 and iteration != args.start_iter:
                    ...
                    cef_net.save_weights(save_path(epoch, iteration))
                    compute_validation_map(epoch, iteration, cef_net, val_dataset, log if args.log else None)
        ...
        if args.interrupt:
            ...
            cef_net.save_weights(save_path(epoch, repr(iteration) + '_interrupt'))
        exit()
...
def compute_validation_map(epoch, iteration, cef_net, dataset, log:Log=None):
    with torch.no_grad():
        cef_net.eval()
        ...
        val_info = eval_script.evaluate(cef_net, dataset, train_mode=True)
        end = time.time()
        logfile = open("logfile.txt", 'a')
